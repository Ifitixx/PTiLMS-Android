...
fun CourseDetailsScreen(
    ...
    Scaffold(
        ...
        Column(
            ...
            when (val currentState = uiState) { // Capture uiState in a local variable
                is CourseDetailsUiState.Loading -> {
                    CircularProgressIndicator()
                }

                is CourseDetailsUiState.Success -> {
                    val courseWithDepartment = currentState.courseWithDepartment // Use the local variable
                    Text(
                ...
                is CourseDetailsUiState.NotFound -> {
                    Text(text = currentState.message) // Use the local variable
                }

                is CourseDetailsUiState.Error -> {
                    Text(
                        text = "Error: ${currentState.message}", // Use the local variable
                        color = Color.Red
                    )
                }
                ...
                is CourseDetailsUiState.SaveError -> {
                    Text(
                        text = "Error saving course details: ${currentState.message}", // Use the local variable
                        color = Color.Red
                    )
                }
            }
